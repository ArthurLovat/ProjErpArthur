inherited frmListagemClientes: TfrmListagemClientes
  Caption = 'Clientes'
  Position = poScreenCenter
  ExplicitWidth = 1100
  ExplicitHeight = 700
  TextHeight = 15
  inherited lblTitulo: TLabel
    Width = 102
    Caption = 'Clientes'
    ExplicitWidth = 102
  end
  inherited grdListagem: TcxGrid
    inherited tbvListagem: TcxGridDBTableView
      object tbvListagemFicha: TcxGridDBColumn
        Caption = 'Ficha'
        PropertiesClassName = 'TcxButtonEditProperties'
        Properties.Buttons = <
          item
            Default = True
            Glyph.SourceDPI = 96
            Glyph.Data = {
              
              110000000049454E44AE426082}
            Kind = bkGlyph
          end>
        Properties.MaxLength = 0
        Properties.ViewStyle = vsButtonsOnly
        Properties.OnButtonClick = tbvListagemFichaPropertiesButtonClick
        Options.ShowEditButtons = isebAlways
      end
      object tbvListagemid: TcxGridDBColumn
        Caption = 'ID'
        DataBinding.FieldName = 'id'
        Options.Editing = False
      end
      object tbvListagemativo: TcxGridDBColumn
        Caption = 'Ativo'
        DataBinding.FieldName = 'ativo'
        Options.Editing = False
      end
      object tbvListagemnome_fantasia: TcxGridDBColumn
        Caption = 'Nome'
        DataBinding.FieldName = 'nome_fantasia'
        Options.Editing = False
        Width = 67
      end
      object tbvListagemrazao_social: TcxGridDBColumn
        Caption = 'Raz'#227'o Social'
        DataBinding.FieldName = 'razao_social'
        Options.Editing = False
        Width = 96
      end
      object tbvListagemcpf_cnpj: TcxGridDBColumn
        Caption = 'CPF/CNPJ'
        DataBinding.FieldName = 'cpf_cnpj'
        Options.Editing = False
      end
      object tbvListagemdata_cadastro: TcxGridDBColumn
        Caption = 'Data De Cadastro'
        DataBinding.FieldName = 'data_cadastro'
        Options.Editing = False
        Width = 117
      end
      object tbvListagemdata_alteracao: TcxGridDBColumn
        Caption = 'Data De Altera'#231#227'o'
        DataBinding.FieldName = 'data_alteracao'
        Options.Editing = False
        Width = 114
      end
      object tbvListagemendereco: TcxGridDBColumn
        Caption = 'Endere'#231'o'
        DataBinding.FieldName = 'endereco'
        Options.Editing = False
        Width = 69
      end
      object tbvListagemnumero_endereco: TcxGridDBColumn
        Caption = 'Numero Endere'#231'o'
        DataBinding.FieldName = 'numero_endereco'
        Options.Editing = False
        Width = 106
      end
      object tbvListagemcomplemento_endereco: TcxGridDBColumn
        Caption = 'Complemento'
        DataBinding.FieldName = 'complemento_endereco'
        Options.Editing = False
        Width = 80
      end
      object tbvListagemponto_referencia: TcxGridDBColumn
        Caption = 'Ponto De Referencia'
        DataBinding.FieldName = 'ponto_referencia'
        Options.Editing = False
        Width = 112
      end
      object tbvListagemcep: TcxGridDBColumn
        Caption = 'Cep'
        DataBinding.FieldName = 'cep'
        Options.Editing = False
      end
      object tbvListagemtcidades_id: TcxGridDBColumn
        Caption = 'ID da Cidade'
        DataBinding.FieldName = 'tcidades_id'
        Options.Editing = False
      end
      object tbvListagemtbairros_id: TcxGridDBColumn
        Caption = 'ID Do Bairro'
        DataBinding.FieldName = 'tbairros_id'
        Options.Editing = False
      end
    end
  end
  inherited dsListagem: TDataSource
    DataSet = DMMain.FDQListagemClientes
    Left = 104
  end
end
